-- =====================================================
-- SPOON - DATOS FINALES DE RESTAURANTES (CORREGIDO)
-- =====================================================
-- Archivo: 11-final-restaurants-data.sql
-- Propósito: Crear categorías y productos con estructura correcta
-- Fecha: 7 de Enero, 2025
-- Versión: 1.0
-- =====================================================

-- Crear categorías globales (sin restaurant_id específico, usando el primer restaurante)
DO $$
DECLARE
    first_restaurant_id UUID;
    user_id UUID;
BEGIN
    SELECT id INTO first_restaurant_id FROM restaurant.restaurants WHERE name != 'Restaurante Demo' LIMIT 1;
    SELECT id INTO user_id FROM auth.users LIMIT 1;
    
    IF first_restaurant_id IS NOT NULL THEN
        INSERT INTO menu.categories (id, restaurant_id, name, description, slug, category_type, color, sort_order, is_active, created_by, updated_by) VALUES 
        (gen_random_uuid(), first_restaurant_id, 'Entradas', 'Sopas y entradas', 'entradas', 'food', '#FF6B35', 1, true, user_id, user_id),
        (gen_random_uuid(), first_restaurant_id, 'Principios', 'Platos principales', 'principios', 'food', '#4ECDC4', 2, true, user_id, user_id),
        (gen_random_uuid(), first_restaurant_id, 'Proteinas', 'Carnes y proteínas', 'proteinas', 'food', '#45B7D1', 3, true, user_id, user_id),
        (gen_random_uuid(), first_restaurant_id, 'Bebidas', 'Bebidas frías y calientes', 'bebidas', 'beverage', '#96CEB4', 4, true, user_id, user_id),
        (gen_random_uuid(), first_restaurant_id, 'Acompanamientos', 'Acompañamientos y postres', 'acompanamientos', 'food', '#FFEAA7', 5, true, user_id, user_id);
    END IF;
END $$;
...
BEGIN
    ...
    SELECT id INTO cat_entradas FROM menu.categories WHERE name = 'Entradas' LIMIT 1;
    SELECT id INTO cat_principales FROM menu.categories WHERE name = 'Principios' LIMIT 1;
    SELECT id INTO cat_bebidas FROM menu.categories WHERE name = 'Bebidas' LIMIT 1;
    SELECT id INTO cat_proteinas FROM menu.categories WHERE name = 'Proteinas' LIMIT 1;
    SELECT id INTO cat_acompanamientos FROM menu.categories WHERE name = 'Acompanamientos' LIMIT 1;
    SELECT id INTO user_id FROM auth.users LIMIT 1;
    
    -- Productos para La Cocina de la Abuela
    IF restaurant_id_1 IS NOT NULL AND cat_entradas IS NOT NULL THEN
    ...
    IF restaurant_id_2 IS NOT NULL AND cat_entradas IS NOT NULL THEN
        INSERT INTO menu.products (
            id, restaurant_id, category_id, name, description, slug,
            base_price, current_price, preparation_time, status,
            created_by, updated_by
    ...
    IF restaurant_id_3 IS NOT NULL AND cat_entradas IS NOT NULL THEN
        INSERT INTO menu.products (
            id, restaurant_id, category_id, name, description, slug,
            base_price, current_price, preparation_time, status,
            created_by, updated_by
    ...
    IF restaurant_id_4 IS NOT NULL AND cat_entradas IS NOT NULL THEN
        INSERT INTO menu.products (
            id, restaurant_id, category_id, name, description, slug,
            base_price, current_price, preparation_time, status,
            created_by, updated_by
    ...
    IF restaurant_id_5 IS NOT NULL AND cat_entradas IS NOT NULL THEN
        INSERT INTO menu.products (
            id, restaurant_id, category_id, name, description, slug,
            base_price, current_price, preparation_time, status,
            created_by, updated_by
