...
const cargarProductosRegex = /useEffect\(\(\) => \{\s*const cargarProductos = async \(\) => \{[\s\S]*?if \(!categoriaId\) \{[\s\S]*?console\.log\('No hay categoría seleccionada, no se cargarán productos'\);[\s\S]*?return;[\s\S]*?\}[\s\S]*?setCargandoProductos\(true\);[\s\S]*?try \{[\s\S]*?localStorage\.removeItem\('menu_productos'\);[\s\S]*?console\.log\('Caché de productos eliminado para forzar recarga'\);[\s\S]*?console\.log\('Intentando cargar productos desde JSON\.\.\.'\);[\s\S]*?const productos = await jsonDataService\.getProductos\(\);/g;

const nuevoCargarProductos = `useEffect(() => {
