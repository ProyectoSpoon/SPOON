...
if (cacheIndicatorRegex.test(content)) {
  ...
  if (cacheTimeEffectRegex.test(modifiedContent)) {
  ...
  if (cacheNotificationEffectRegex.test(finalContent || modifiedContent)) {
    ...
    const contentToModify = finalContent || modifiedContent;
    const finalContentWithNotification = contentToModify.replace(
  ...
  const lastUseEffectIndex = (finalContentWithNotification || finalContent || modifiedContent).lastIndexOf('useEffect(');
  const lastUseEffectClosingBrace = (finalContentWithNotification || finalContent || modifiedContent).indexOf('}, [', lastUseEffectIndex);
  const insertionPoint = (finalContentWithNotification || finalContent || modifiedContent).indexOf(');', lastUseEffectClosingBrace) + 2;
  
  const contentWithDebug = [
    (finalContentWithNotification || finalContent || modifiedContent).slice(0, insertionPoint),
    debugCode,
    (finalContentWithNotification || finalContent || modifiedContent).slice(insertionPoint)
  ].join('');
