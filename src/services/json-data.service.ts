...
    
    if (!response.ok) {
      console.error(`Error al cargar productos desde API: ${response.status}`);
      return [];
    }
    
    const data = await response.json();
    const productosCargados = data.productos || [];
    
    console.log(`API: Cargados ${productosCargados.length} productos para categoría ${categoriaId}`);
    
...
    const productosTransformados = productosCargados.map((prod: any) => {
      // Aseguramos que se establezca categoriaId correctamente
      const categoriaIdValue = prod.categoriaId || prod.id_categoria || categoriaId;
    
      console.log(`Transformando producto ${prod.nombre}: categoriaId original=${prod.categoriaId || prod.id_categoria}, asignado=${categoriaIdValue}`);
...
        categoriaId: categoriaIdValue, // <--- CAMBIO AQUÍ
...
        const productosAsociados = productosPorCombinacion[comb.id_combinacion] || {}; // Renombrada variable para claridad
        return {
...
          cantidad: 0,
          entrada: productosAsociados.entrada || null,
          principio: productosAsociados.principio || null,
          proteina: productosAsociados.proteina || null,
          acompanamiento: productosAsociados.acompanamiento ? [productosAsociados.acompanamiento] : [],
          bebida: productosAsociados.bebida || null,
