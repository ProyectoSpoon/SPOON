...
interface User {
  ...
  restaurantId?: string;    // ← NUEVO: ID del restaurante del usuario
}
...
interface AuthContextType {
  ...
  restaurantId?: string; // ← NUEVO: Propiedad directa en el contexto para acceso rápido
}
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const mapUserFromDB = (dbUser: any, permissions: string[] = []): User => {
    return {
      ...
      permissions: permissions.length > 0 ? permissions : getPermissionsByRole(dbUser.role),
      restaurantId: dbUser.restaurantId || dbUser.restaurant_id // ← NUEVO: Incluir restaurantId si existe
    };
  };
  ...
  const login = async (email: string, password: string) => {
    try {
      ...
      const userInfoForCookie = {
        ...
        restaurantId: data.user.restaurantId || data.user.restaurant_id // ← Incluir restaurantId si existe
      };
  ...
  // NUEVO: Función para actualizar restaurantId
  const updateRestaurantId = (restaurantId: string) => {
    if (!user) {
      console.warn('⚠️ No hay usuario logueado, no se puede actualizar restaurantId');
      return;
    }

    console.log('🏪 Actualizando restaurantId:', restaurantId);

    // Actualizar estado del usuario
    const updatedUser = { ...user, restaurantId };
    setUser(updatedUser);

    // Actualizar cookie user-info con restaurantId
    const userInfoForCookie = {
      uid: user.id,
      id: user.id,
      email: user.email,
      ...
      restaurantId: restaurantId // ← Incluir restaurantId en cookie
    };

    const cookieMaxAge = 7 * 24 * 60 * 60; // 7 días
    document.cookie = `user-info=${encodeURIComponent(JSON.stringify(userInfoForCookie))}; path=/; max-age=${cookieMaxAge}; secure; samesite=strict`;

    // También actualizar localStorage como backup
    localStorage.setItem('user_info', JSON.stringify(updatedUser));

    console.log('✅ RestaurantId actualizado en contexto, cookies y localStorage');
  };
  ...
  return (
    <AuthContext.Provider value={{
      ...
      hasAnyPermission,
      updateRestaurantId,
      restaurantId: user?.restaurantId // ← Exponer ID directamente
    }}>
      {children}
