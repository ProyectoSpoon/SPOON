...
// Tipos adicionales para festivos
export interface Festivo {
  id: string;
  nombre: string;
  fecha: Date;
  tipo: 'nacional' | 'regional' | 'local' | 'personalizado';
  activo: boolean;
  descripcion?: string;
...
// Tipos adicionales para configuración de horarios
export interface ConfiguracionHorario {
  id: string;
  nombre: string;
  horarios: Horario[];
  horariosPorDefecto: { [key: string]: RangoHorario[] };
  activo: boolean;
  fechaCreacion: Date;
  fechaModificacion: Date;
}

export type DiaSemana = Dia;
