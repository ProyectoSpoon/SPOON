...
const IconShowcase = () => {
  ...
  const createIcon = (paths: React.ReactNode) => {
    const IconComponent = ({ size = 24, variant = 'default' as LocalVariant }) => {
              ...
              <div className="grid grid-cols-3 gap-2">
                {variants.map(variant => (
                  <button
                    ...
                    className={`flex items-center space-x-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${selectedVariant === variant.key
                        ? 'bg-spoon-primary text-white'
                        : 'bg-spoon-neutral-100 text-spoon-neutral-600 hover:bg-spoon-neutral-200'
                      }`}
                  >
