...
export async function GET(request: NextRequest) {
  try {
    ...
    if (tipo) {
      queryText += ` AND c.tipo = $${paramIndex}`;
      params.push(tipo);
      paramIndex++;
    }

    queryText += ' ORDER BY c.orden ASC, c.nombre ASC';
  ...
  try {
    ...
    const queryText = `
      INSERT INTO categorias (
        nombre, tipo, orden, descripcion, restaurante_id,
    ...
    const categoria = {
      ...
      activo: newCategory.activo,
      restauranteId: newCategory.restaurante_id,
