...
const LoginPage = () => {
  ...
  const manejarEnvio = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // ✅ VERIFICAR CONFIGURACIÓN COMPLETA DEL RESTAURANTE
      // ✅ VERIFICAR CONFIGURACIÓN COMPLETA DEL RESTAURANTE
      console.log('🔍 Redirigiendo al dashboard (El Guardián validará el estado)...');

      // Permitimos que el dashboard/layout.tsx maneje la redirección exacta
      ...
      // Friendly Check: Lógica de usuario no encontrado
      if (mensajeError.includes('Invalid login credentials')) {
        setError('El usuario no existe. ¿Eres nuevo?');
        // No mostramos toast.error para no ser redundantes, el UI lo manejará
      } else {
  ...
  const toggleModoRegistro = () => {
    ...
    setError(null);
    setDatosFormulario({ correo: '', contrasena: '' });
  };

  return (
    <div className="flex min-h-screen">
      {/* Toaster eliminado de aquí, ya está en providers.tsx */}
      ...
      <div className="w-full md:w-1/2 bg-white flex items-center justify-center p-8">
        {/* Condicional: Mostrar formulario de registro o login */}
        {modoRegistro ? (
          <FormularioRegistro onBackToLogin={() => setModoRegistro(false)} />
        ) : (
          /* FORMULARIO DE LOGIN */
          <div className="w-full max-w-md space-y-8">
            ...
            {error && (
              <div className={`p-4 rounded border-l-4 ${error.includes('¿Eres nuevo?') ? 'bg-blue-50 border-blue-500' : 'bg-red-50 border-red-500'}`}>
                <p className={`text-sm ${error.includes('¿Eres nuevo?') ? 'text-blue-700' : 'text-red-700'}`}>
                  {error}
                  {error.includes('¿Eres nuevo?') && (
                    <span className="block mt-1">
                      <Link href="/registro" className="font-semibold underline hover:text-blue-900">
                        Regístrate aquí
                      </Link>
                    </span>
                  )}
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded">
              <p className="text-blue-700 text-sm">
                ...
                Email: <strong>carlosalberto.rodriguezjimenez@gmail.com</strong> <br />
                Contraseña: <strong>(Tu contraseña)</strong>
              </p>
            </div>
            ...
            <div className="text-center space-y-4">
              <p className="text-sm text-gray-600">
                ...
                <button
                  ...
                  className="font-medium text-spoon-primary hover:text-spoon-primary transition-colors"
                >
                  Crear cuenta
                </button>
              </p>
