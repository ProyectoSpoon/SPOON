...
// ✅ OPTIMIZACIÓN 1: Componente memoizado para filas individuales
const ProductRow = React.memo(({ 
  ...
  // ✅ OPTIMIZACIÓN 2: Callbacks memoizados para prevenir re-renders
  const onAgregar = React.useCallback(() => {
    handleAgregarAlMenu(producto);
  }, [handleAgregarAlMenu, producto]);

  const onVerDetalles = React.useCallback(() => {
    handleViewProductDetails(producto);
  }, [handleViewProductDetails, producto]);
  ...
  const esFavorito = React.useMemo(() => isFavorito(producto.id), [isFavorito, producto.id]);

  return (
    <div className="grid grid-cols-12 items-center py-2 hover:bg-gray-50">
      {/* Imagen */}
      ...
      <div className="col-span-3 flex justify-center">
        <button
          className="px-3 py-1 bg-spoon-primary hover:bg-spoon-primary-dark text-white text-xs rounded-md transition-colors"
          onClick={onAgregar}
        >
          Agregar
        </button>
      ...
      <div className="col-span-2 flex justify-center space-x-2">
        ...
        <button
          className={`p-1 rounded-md hover:bg-gray-100 transition-colors ${
            esFavorito ? 'text-red-500' : 'text-gray-400 hover:text-red-400'
          }`}
          onClick={onToggleFavorite}
          title={esFavorito ? "Quitar de favoritos del restaurante" : "Agregar a favoritos del restaurante"}
          disabled={favoritosLoading}
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill={esFavorito ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
...
// ✅ OPTIMIZACIÓN 3: Tabla principal memoizada
export const ProductTable = React.memo(({ 
  ...
  // ✅ OPTIMIZACIÓN 4: Memoizar filtrado pesado
  const productosFiltrados = React.useMemo(() => {
    return versionedProductosSeleccionados.filter(
      (producto: VersionedProduct) => producto.categoriaId === selectedCategoryTab
    );
  }, [versionedProductosSeleccionados, selectedCategoryTab]);

  // ✅ OPTIMIZACIÓN 5: Memoizar mensaje de categoría vacía
  const showEmptyMessage = React.useMemo(() => {
    return productosFiltrados.length === 0 && versionedProductosSeleccionados.length > 0;
  }, [productosFiltrados.length, versionedProductosSeleccionados.length]);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm mx-4">
      <div className="overflow-hidden rounded-lg border border-gray-200" style={{height: '320px', overflow: 'auto'}}>
        ...
        <div className="grid grid-cols-12 bg-gray-50 text-xs font-medium text-gray-500 border-b border-gray-200 sticky top-0">
        ...
        <div className="divide-y divide-gray-200">
          ...
          {/* Mensaje optimizado si no hay productos */}
          {showEmptyMessage && (
